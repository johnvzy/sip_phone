	...
	private final View.OnClickListener mOnDialerClick = new View.OnClickListener() {
		...
		public void onClick(View v) {
			...
			if(tag == DialerUtils.TAG_CHAT){
				...
				if(mSipService.isRegistered() && !NgnStringUtils.isNullOrEmpty(number)){
					Log.v("按下通話鍵，開始撥號","->Voice Call");
					String selfId=ScreenHome.LastRegBedId;
					String autoComplete="";
					if (selfId.trim().length()>0)
					{
						autoComplete=selfId.substring(0,4);
						Log.v("自動補號：",autoComplete);
					}
					ScreenAV.makeCall(autoComplete + number, NgnMediaType.Audio);
					mEtNumber.setText(NgnStringUtils.emptyValue());
				}
			}
